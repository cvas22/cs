...
* Time: 1:57 AM
*
* This script generates average, high, and low values for each bathroom type
*/
...
$SelectQ = "SELECT totbath, ceil(avg(listprice)), min(listprice), max(listprice) from residential where city='$data[1]' AND zip='$data[2]' AND county='$data[3]' AND status='Active' GROUP BY totbath";
...
while($myDat = mysqli_fetch_array($res, MYSQLI_NUM))
	{
		...
		$InsertQuery = "INSERT INTO stats_bath (`stats_geo_id`,`zip`,`city`,`county`,`state`,`bath_count`,`average`,`lowest`,`highest`,`timestamp`,`on_market`) VALUES ('$data[0]', '$data[2]', '$data[1]', '$data[3]', '$data[4]', '$myDat[0]', '$myDat[1]', '$myDat[2]', '$myDat[3]', now(), 1)";
		//echo $InsertQuery . '<br>';
