...
* Time: 1:57 AM
*
* This script generates average, high, and low values for each garage type
*/
...
$SelectQ = "SELECT capgarage, ceil(avg(listprice)), min(listprice), max(listprice) from residential where city='$data[1]' AND zip='$data[2]' AND county='$data[3]' AND status='Active' GROUP BY capgarage";
...
while($myDat = mysqli_fetch_array($res, MYSQLI_NUM))
	{
		...
		$InsertQuery = "INSERT INTO stats_garage (`stats_geo_id`,`zip`,`city`,`county`,`state`,`garage_count`,`average`,`lowest`,`highest`,`timestamp`,`on_market`) VALUES ('$data[0]', '$data[2]', '$data[1]', '$data[3]', '$data[4]', '$myDat[0]', '$myDat[1]', '$myDat[2]', '$myDat[3]', now(), 1)";
		//echo $InsertQuery . '<br>';
