...
$query_city = "select distinct city from stats_geo";
$result_city = $conn->query($query_city);
if(!$result_city) die($conn->error);
$conn = new mysqli($hn, $un, $pw, $db);
while($data=mysqli_fetch_array($result_city, MYSQLI_NUM)) {
$query = "INSERT INTO listings_city_median 
SELECT 'id',t1.city, avg(t1.listprice) as median_val, now() FROM (
SELECT @rownum:=@rownum+1 as `row_number`, d.listprice,d.city
