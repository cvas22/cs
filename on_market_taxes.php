...
* Time: 1:57 AM
*
* This script generates average, high, and low values of taxes paid per city, county, zip, and state
*/
...
$SelectQ = "SELECT ceil(avg(taxes)) as average_taxes,
       ceil(min(taxes)) as min_taxes,
       ceil(max(taxes)) as max_taxes
...
while($myDat = mysqli_fetch_array($res, MYSQLI_NUM))
	{
		...
		$InsertQuery = "INSERT INTO stats_taxes (`stats_geo_id`,`zip`,`city`,`county`,`state`,`average`,`lowest`,`highest`,`timestamp`,`on_market`) VALUES ('$data[0]', '$data[2]', '$data[1]', '$data[3]', '$data[4]', '$myDat[0]', '$myDat[1]', '$myDat[2]', now(), 1)";
		//echo $InsertQuery . '<br>';
